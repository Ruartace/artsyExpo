...
const BASE_URL = import.meta.env.VITE_API_BASE_URL ??  'http://u49dd496.natappfree.cc'
...
async function request<T>(method: 'GET' | 'POST', url: string, opts: { params?: Record<string, unknown>, body?: unknown, headers?: Record<string, string>, timeout?: number } = {}): Promise<HttpResponse<T>> {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && 'code' in (parsed as Record<string, unknown>) && 'message' in (parsed as Record<string, unknown>)) {
...
export async function httpPost<T>(url: string, body?: unknown, headers?: Record<string, string>): Promise<HttpResponse<T>> {
...
export async function checkTunnel(path: string = '/') {
  ...
  try {
    ...
    const res = await fetch(url, { method: 'GET', signal: controller.signal, credentials: 'omit', cache: 'no-store' })
    clearTimeout(timer)
